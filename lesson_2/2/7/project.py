...
def create_function_button(text, command, args=None):
    return tk.Button(controls_frame, width=30, text=text, command=lambda: do_command_with_thread(command, args))
...
history_btn = tk.Button(save_and_history_frame, text="History", command=show_history)
